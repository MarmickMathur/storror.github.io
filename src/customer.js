...
onAuthStateChanged(customerauth, (user) => {
  ...
  let search = document.getElementsByClassName("searchbars")[0];
  let searchbtn = search.nextElementSibling;
  let search2 = document.getElementsByClassName("searchbars")[1];
  let searchbtn2 = search2.nextElementSibling;
  ...
  function renderingopenstores() {
    for (let index = 0; index < stores.length; index++) {
      ...
      if (openstores.includes(store)) {
        document.getElementById(store + "card").style.display = "flex";
      } else {
  ...
  function fillingopenstores() {
    openstores = [];
    controllarray.forEach((element) => {
      if (element.status) {
        openstores.push(element.name);
      }
    });
  }
  ...
  function fillingcontrollarray(snapshot) {
    controllarray = [];
    for (const vendor in snapshot.docs) {
      if (Object.hasOwnProperty.call(snapshot.docs, vendor)) {
        const doc = snapshot.docs[vendor];
        for (const storename in doc.data()) {
          if (Object.hasOwnProperty.call(doc.data(), storename)) {
            const store = doc.data()[storename];
            let vendorstorepair = {
              vendor: doc.id,
              name: storename,
            };
            vendrosortedarray.push(vendorstorepair);
  ...
  async function displaydata(element) {
    ...
    for (const key in element.data()) {
      if (Object.hasOwnProperty.call(element.data(), key)) {
        ...
        let vendorid = element.id.split("@")[0];

        let attr = JSON.stringify(metadata);
        // console.log(attr);
        // making a metadata for the store id
      ...
      <div class = " flex  justify-between">
          ...
          <div class = "text-xl mt-1 w-fit uppercase pl-2 font-normal text-gray-400 ">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 inline-block h-6">
          ...
          <div class = "border-2 border-white text-center bg-transparent  m-2 rounded-md flex flex-row">
            ...
            <div class = "w-[50%] flex justify-evenly flex-row">
              <div class=" w-fit" >
              <form id = "${voteformid}" class = " flex h-full w-fit ml-2  justify-center"> 
              <label class = " cursor-pointer">
                <input type = "radio" class = "hidden peer" attr = ${attr} name = "voting" id = "${upvoteid}" value = "true">
              ...
              <label class = " cursor-pointer">
                <input type = "radio" class = "hidden peer" attr = ${attr} name = "voting" id = "${downvoteid}" value = "false">
                <div class = " bg-transparent text-white hover:text-black px-5 hover:bg-white h-full text-center peer-checked:text-black peer-checked:bg-white">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
              ...
              <div attr = ${attr} class = "hover:bg-white ml-5 hover:text-black  cursor-pointer deletevotes text-white p-2 w-fit h-full">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
    ...
    for (const key in element.data()) {
      if (Object.hasOwnProperty.call(element.data(), key)) {
        ...
        menu += `<tr class = "w-full" >
                      <td class = "w-[50%] overflow-hidden">${ele.name}</td>
  ...
  function deletevotes(user, users, main) {
    let uvoterid = user.email.replace("@gmail.com", "");
    let btns = document.getElementsByClassName("deletevotes");

    for (const key in btns) {
      if (Object.hasOwnProperty.call(btns, key)) {
        const btn = btns[key];
        btn.addEventListener("click", () => {
          let metadata = JSON.parse(btn.getAttribute("attr"));
          console.log(metadata);
          let store = doc(customerdb, "votes", metadata.storeid);

          getDoc(store).then((element) => {
            if (typeof element.data()[uvoterid] != "undefined") {
              updateDoc(store, {
                [uvoterid]: deleteField(),
              }).then(() => {
  ...
  if (user) {
    ...
    onSnapshot(
      ...
      (main = async (snapshot) => {
        ...
        // search bars for id of store
        search.addEventListener("keyup", () => {
          let value = search.value;
          console.log(value);
          for (let index = 0; index < controllarray.length; index++) {
            const element = controllarray[index];
            console.log(element);
            let storename = element.storename;
            let cardid = element.name + "card";
            if (storename.includes(value)) {
        ...
        // search bar for vendor
        search2.addEventListener("keyup", () => {
          let input = search2.value;
          let cardid;
          for (let index = 0; index < controllarray.length; index++) {
            const element = vendrosortedarray[index];
            let vendorname = element.vendor;
            let storename = element.name;
            cardid = element.name + "card";
            if (vendorname.includes(input)) {
              if (openstores.includes(storename)) {
                document.getElementById(cardid).style.display = "flex";
              } else {
                document.getElementById(cardid).style.display = "none";
              }
    ...
    document
      .getElementsByClassName("sortmenu")[0]
      .addEventListener("click", function ff() {
        document.getElementsByClassName("sortoptions")[0].style.display =
          "flex";
        document
          .getElementsByClassName("sortmenu")[0]
          .removeEventListener("click", ff);
        document
          .getElementsByClassName("sortmenu")[0]
          .addEventListener("click", function yy() {
            ...
            document
              .getElementsByClassName("sortmenu")[0]
              .removeEventListener("click", yy);
            document
              .getElementsByClassName("sortmenu")[0]
              .addEventListener("click", ff);
          });
      });
