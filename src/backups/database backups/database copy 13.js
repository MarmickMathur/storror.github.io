...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    let visiblity = {};
    // ---------------------------------------------
    document.getElementById("userid").innerHTML += user.email.replace(
      "@gmail.com",
      ""
    );
      ...
      <form id="addstore" class="bg-transparent shadow-lg">
        ...
        <div class=" justify-between  border-white m-9 border-2 overflow-hidden rounded-md">
            ...
            <div class = "flex border-b-2 border-white">
                    ...
                    <div class = "w-[70%] flex border-l-2 border-white p-2">
    ...
    function updatingdata(element, visiblity) {
      ...
      <div class="p-3 relative bg-gray-900 rounded-b-md shadow-lg z-50 shadow-black">
          ...
          <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
             ...
             <span class = " text-center  ">${ele.adress}</span>
          </p>
          ...
          <form action="" id="${ele.name}getmenu"
                ...
                <input id = "itemname" type="text" name="itemname" class="w-1/2 mr-5 bg-transparent rounded-md border-2 border-white h-12 text-white" placeholder="ADD ITEMS HERE"  required autocomplete="off">
                  <div class=" w-1/2 flex justify-between items-center">
                    <label for="${ele.name}available" class = " p-2 rounded-md border-2 border-white h-full w-1/2 hover:bg-white hover:text-black m-2 text-white text-center bg-transparent  peer-checked:text-gray-800 peer-checked:bg-white">available</label>
                      <input type="radio" name="availablity" id="${ele.name}available" class= "peer" value="true">
                    <label for="${ele.name}unavailable" class = " p-2 bg-transparent  text-white w-1/2 rounded-md hover:bg-white hover:text-black border-2 border-white h-full m-2 text-center "> unavailable</label>
                      <input type="radio" name="availablity" id="${ele.name}unavailable" class= "peer" value="false">
                  </div>
                </div>
      ...
      <div class = "commentloader bg-white relative z-10 transition-all -translate-y-5 rounded-b-md hover:-translate-y-1 " >
      <div>some comments</div>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 m-5 inline-block h-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 13.5L12 21m0 0l-7.5-7.5M12 21V3" />
      </svg>
      <p class = " text-center font-headingside2 font-semibold text-lg text-black inline-block mt-8  uppercase "> comments </p>
      </div>
      </div>
      ` + submod.innerHTML;
        }
      ...
      for (const key in element.data()) {
        if (Object.hasOwnProperty.call(element.data(), key)) {
          ...
          commentlist.innerHTML =
            ...
            <div class = " text-sm text-white pl-2">${ele.comment}</div></div>`;
        }
      }
    }
      ...
      for (const key in element.data()) {
        if (Object.hasOwnProperty.call(element.data(), key)) {
          ...
          menu += `<tr >
                        ...
                        <td class = "w-40% overflow-hidden"><span class = "statechanger hover:text-red-500 cursor-pointer" metadata = '${attr}'>${availablity}</span></td>
                        <td class = "w-20%">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" metadata='${attr}' class="w-6 menudelete cursor-pointer hover:stroke-red-600 h-6">
      ...
      for (const key in btns) {
        if (Object.hasOwnProperty.call(btns, key)) {
          ...
          ele.addEventListener("click", (e) => {
            let jsonobejct = JSON.parse(ele.getAttribute("metadata"));
      ...
      for (const key in btns) {
        if (Object.hasOwnProperty.call(btns, key)) {
          ...
          ele.addEventListener("click", (e) => {
            let jsonobejct = JSON.parse(ele.getAttribute("metadata"));
            let menutobeupdated = doc(vendordb, "menus", jsonobejct.name);
            getDoc(menutobeupdated).then((snapshot) => {
              let menu = snapshot.data()[jsonobejct.item];
              console.log(menu.availablity, menu.createdat, menu);
              updateDoc(menutobeupdated, {
                [jsonobejct.item]: {
    ...
    onSnapshot(qusers, function show(snapshot) {
      cleardata();
