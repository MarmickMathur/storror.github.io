...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    function updatingdata(element) {
      for (const key in element.data()) {
        if (Object.hasOwnProperty.call(element.data(), key)) {
          ...
          // providing menu tab
          let menutab = "";
          for (const key2 in ele.menu) {
            if (Object.hasOwnProperty.call(ele.menu, key2)) {
              const element = ele.menu[key2];
              if (element) {
                menutab += `<tr>
                            <td>${key2}</td>
                            <td>available</td>
          ...
          submod.innerHTML += `<div id="${modalid}" class="modalcard hidden">
      <img src="/images/store image.jpg" alt="canteenimage" class="h-3/4 w-full object-cover ">
      <div class="m-3">
          <p id="" class="hidden text-left text-2xl text-gray-200 font-headingside1 uppercase font-bold">
              ${ele.name} </p>
    ...
    function addingiteamstothemenu(e, menu) {
      e.preventDefault();
      let availability = check(getmenu.availability.value);
      menu[getmenu.getiteam.value] = availability;
    }
