...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    function cleardata() {
      ...
      submod.innerHTML = `<div id="modalform" class="modalcard ${visiblity["modalform"]}">
        <div class="m-3 justify-between flex flex-col">
            ...
            <form action="" id="getmenuform"
                ...
                <input id = "itemname" type="text" name="itemname" class="" required>
                <div class="my-5">
      ...
      let getmenu = document.getElementById("getmenuform");
      getmenu.addEventListener("submit", (e) => {
        e.preventDefault();
        console.log("getmenuform");
      });
    }
    ...
    function updatingdata(element, visiblity) {
      ...
      <div class="m-3">
          ...
          <form action="" id="${ele.name}getmenu"
                class=" getmenu bg-white rounded-md mt-5 flex flex-col overflow-hidden cursor-default p-5 h-full">
                <h3 class="mb-2">add menus to the store that is submited</h3>
                <input id="${ele.name}itemname" type="text" name="itemname" class="" required>
                <div class="my-5">
                    <label for="availablity">availablity:</label>
                    <label for="available">available</label>
                    <input type="radio" name="availablity" id="available" value="true">
    ...
    function addingstore() {
      ...
      console.log(visiblity, "visisblity after the store is added");
    }

    // -----------------------------------------------------------------------------
    onSnapshot(qmenus, function printmenu(snapshot) {
      // for menus
    ...
    Mod.addEventListener("click", () => {
      ...
      console.log("clicked on blur back gorund");
